angular.module('App').controller('RootCtrl', function ($rootScope, $scope, $mdSidenav, $mdToast, $mdDialog, $cookies, request, focus) {
	...
	root.PANEL_NAME = "Odysée";
	root.PANEL_VERSION = "1.1";

	/*Constant String data */
	root.PRODUCT_UPDATE = "modifier produit";
	root.PRODUCT_NEW = "Nouvelle Produit";
	...
	self.sidenav = {
		actions: [
			...
			{ name: 'Produits', icon: 'widgets', link: '#product', sub: false },
			{ name: 'Catalogue', icon: 'dns', link: '#category', sub : false },
			{ name: 'Nouvelle Produit', icon: 'subject', link: '#news', sub: false },
	...
	root.getSessionUid = function () { return $cookies.get(self.uid_key); };
	root.getSessionName = function () { return $cookies.get(self.uid_name); };
	root.getSessionEmail = function () { return $cookies.get(self.uid_email); };
