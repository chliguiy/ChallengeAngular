...
public function login(){
   if($this->get_request_method() != "POST") $this->response('',406);
        if(!isset($_REQUEST['username'])) $this->responseInvalidParam();
...
if(!empty($username) and !empty($password)){ // empty checker
            ...
            if($r->num_rows > 0) {
                ...
                $resp = array('status' => "success", "user" =>$this->db->get_one($query));
    ...
    public function findAllClient(){
        ...
        $offset = ($page * $limit) - $limit;
        $count_total = $this->clients->allCountPlain($q, 1);
        $clients = $this->clients->findAllByPagePlain($limit, $offset, $q, 1);
        ...
        $count = count($clients);
        $response = array(
            'status' => 'success', 'count' => $count, 'count_total' => $count_total, 'pages' => $page, 'client' => $object_res
        );
        $this->show_response($response);
    }
